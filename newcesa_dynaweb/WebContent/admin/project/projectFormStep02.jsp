...
<%
	...
	if(!"".equals(seq)){
		...
		if(cRowSet.next()){
			...
			if(impl.equals("")) {
				sbp_ProjectName = "SBP 프로젝트를 선택해주세요.";
			} else {
...
<div id="form_warp">
	<dl>
		...
		<dt id="pjn" class="inpt_dt">프로젝트명</dt>
			...
			if(sbp_activityId == "" || sbp_activityId == undefined) {
				...
				$(this).append(htmlCode += '<input type="hidden" name="tempActImage" id="tempImage'+tds+'" value="' + activityImage + '" />' + 
								...
								'<form name="activityFileForm" method="post" enctype="multipart/form-data" onSubmit="onSubmitForm(this); return false;">' +
									...
									'<input type="submit" class="button blue" value="업로드" style="width:50px"/>' +
								'</form>' +
...
$(window).load(function() {
	...
	/* SBP프로젝트만 연결되어있을 경우 */
	if((!sbp_ProjectName.equals("SBP 프로젝트를 선택해주세요.")) && (count == 0)) {
	...
	} else if((!sbp_ProjectName.equals("SBP 프로젝트를 선택해주세요.")) && (count > 0)) {
	%>
		var htmlCode = "<%=html.toString()%>";
		$("#sbp_ProjectName").attr("class", "");
		$("#sbp_ProjectName").attr("style", "");	
...
function connectSBP_Project(target) {
	...
	$("#spp").val(spp);	
	$("#seq").val(seq);
	frm2.attr('action','projectProcStep02.jsp').submit();
}
...
function SBP_disConnect_yes() {
	...
	frm3.attr('action','projectProcStep02.jsp').submit();
}
...
function SBP_Project_disConnect_yes() {
	...
	var frm4 = $("#frm4");
	frm4.attr("action", "projectProcStep02.jsp").submit();
}
...
<div class="btn_group_left_align" style="float: right">
	<input  type="button" onclick="addCol('', '/images/blank.gif', '');" value="Activity 추가" class="button blue" />
	<input  type="button" onclick="showSBPList();" value="SBP Activity 추가" class="button blue" />
</div>
<br/>
<br/>
<br/>
...
<div id="SBP_List_Modal" class="modal" style="display:none;">
	<div class="sbp-list-modal-content">
		...
		<button type="button" class="button blue sact" style="width:35px;" onclick="SBP_Activity_Connect();">확인</button>
	</div>
</div>
...
<div id="SBP_Map_Modal" class="modal" style="display:none;">
	<div class="sbp-map-modal-content">
		...
		<div id="button_position" style="text-align:center;">
			<button type="button" class="button blue" style="width:100px;" onclick="SBP_disConnect();">SBP 연결 해제</button>
		</div>
	</div>
</div>
...
<div id="SBP_DisConnect_Modal" class="modal" style="display:none;">
	<div class="sbp-disconnect-modal-content">
		<div style="text-align:center;">해당되는 activity까지 모두 삭제됩니다.</div> 
		<div style="text-align:center; margin-top:5px;">계속 하시겠습니까?</div>
		<div style="margin-top:20px; text-align:center;">
			<button type="button" class="button blue" style="width:35px;" onclick="SBP_disConnect_yes();">예</button>
			<button type="button" class="button blue" style="width:40px;" onclick="SBP_disConnect_no();">아니오</button>
		</div>
	</div>
</div>
...
<div id="SBPProject_DisConnect_Modal" class="modal" style="display:none;">
	<div class="sbpproject-disconnect-modal-content">
		<div style="text-align:center;">SBP Project와의 연결을 해제합니다.</div> 
		<div style="text-align:center; margin-top:5px;">계속 하시겠습니까?</div>
		<div style="margin-top:20px; text-align:center;">
			<button type="button" class="button blue" style="width:35px;" onclick="SBP_Project_disConnect_yes();">예</button>
			<button type="button" class="button blue" style="width:40px;" onclick="SBP_Project_disConnect_no();">아니오</button>
		</div>
	</div>
</div>
