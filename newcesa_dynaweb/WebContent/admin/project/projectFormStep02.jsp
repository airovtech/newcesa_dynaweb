...
function addCol(activityText, activityImageFullpath, activityImage){
	...
	$('#activityTable tr').each(function()
    {
		count++;
		if(count==1){
			$(this).append('<th><input type="text" name="activityName" id="activityName'+tds+'" size="10" value="'+activityText+'" title="'+activityText+'" onchange="ovrlapChk(this)"/><br/>' + 
								'<input type="hidden" name="tempActImage" id="tempImage'+tds+'" value="' + activityImage + '" />' + 
...
$(window).load(function() {
	...
	%>
		addCol("<%=actName%>", "<%=actImageFullpath%>", "<%=actImage%>");
	<%
	}
	%>
	printWord(<%=sbufWord%>);
