<%@ page language="java" contentType="text/html; charset=utf-8"%><%@ include file="/include/header.jsp" %><%@ include file="/include/needAdminLogin.jsp" %><%
	...
	try{
		...
		sbufTableChecked.append("<th colspan=2>경험어휘</th>");
		sbufTableChecked.append("<th>No.</th>");
		sbufTableChecked.append("<th colspan=2>경험어휘</th>");
		sbufTableChecked.append("</tr>");
		sbufTableChecked.append("</thead>");
		...
		if(count==0){
			if("0".equals(groupSeq)){
				sbufTableChecked.append("<tr><td colspan='6'>그룹을 선택하세요.</td></tr>");
			}
			else{
				sbufTableChecked.append("<tr><td colspan='6'>등록된 어휘가 없습니다.</td></tr>");
			}
		}
		else{
			if(count%2==1){
		...
		count=0;
		cRowSet.beforeFirst();
		sbufTable.append("<div style='padding-top:20px;padding-bottom:10px;font-weight:bold'>추가 어휘</div>");
		...
		sbufTable.append("<th colspan=2>경험어휘</th>");
		sbufTable.append("<th>No.</th>");
		sbufTable.append("<th colspan=2>경험어휘</th>");
		sbufTable.append("</tr>");
		sbufTable.append("</thead>");
		...
		if(count==0){
			if("0".equals(groupSeq)){
				sbufTable.append("<tr><td colspan='6'>그룹을 선택하세요.</td></tr>");
			}
			else{
				sbufTable.append("<tr><td colspan='6'>등록된 어휘가 없습니다.</td></tr>");
			}
		}
		else{
			if(count%2==1){
