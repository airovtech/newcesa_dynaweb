...
<%
	String activityIndex = StringUtils.defaultString(request
				.getParameter("activityIndex"));
		...
		try {
			...
			pstmt2 = conn
					.prepareStatement("select * from (SELECT SUM(check_value) as A FROM member_check where memberid="
							+ memberid2
							+ " && word='자연스런' && activity='A') a,(SELECT SUM(check_value) as B FROM member_check where memberid="
							+ memberid2
							+ " && word='자연스런' &&activity='B') b,	(SELECT SUM(check_value) as C FROM member_check where memberid="
							+ memberid2
							+ "&& word='자연스런'  && activity='C') c,(SELECT SUM(check_value) as D FROM member_check where memberid="
							+ memberid2
							+ "&& word='자연스런'  && activity='D') d,(SELECT SUM(check_value) as E FROM member_check where memberid="
							+ memberid2
							+ "&& word='자연스런'  && activity='E') e;");
